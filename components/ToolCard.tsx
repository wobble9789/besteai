    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow flex flex-col">
      ...
      <div className="flex items-center gap-1 mb-4">
        {[1, 2, 3, 4, 5].map((star) => (
          <svg
            key={star}
            className={`w-4 h-4 ${star <= Math.round(tool.beoordeling) ? "text-yellow-400" : "text-gray-200"}`}
            fill="currentColor"
            viewBox="0 0 20 20"
          >
        ...
        <span className="text-sm text-gray-500 ml-1">{tool.beoordeling}/5</span>
      </div>

      <div className="flex items-center justify-between mb-5">
        <span className="text-lg font-bold text-gray-900">{tool.prijs}</span>
        <div className="flex gap-1">
          {tool.categorieen.slice(0, 2).map((cat) => (
